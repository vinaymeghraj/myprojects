...
public class SampleConsumerPrint {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            ...
            if (records.isEmpty())
                System.out.println("Consumer " + "-" + " *==*==*==*==*==* Poll consumed zero messages !! *==*==*==*==*==*");
            else {
                System.out.println("Consumer poll consumed " + records.count() + " messages !!");
                for (ConsumerRecord<String, String> record : records) {
                    //If the published message is in String Format then just print message using record.value()
                    System.out.printf(" timestamp = %s, offset = %d, key = %s, partition = %s, value = %s%n", formatter.format(date), record.offset(), record.key(), record.partition(), record.value());

                    //If the published message is in JSON Format then just print message using ObjectMapper
                    /*try {
                    ObjectMapper mapper = new ObjectMapper();
